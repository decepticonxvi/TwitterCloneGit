...
$(document).ready(() => {
  ...
  $.get(`/api/chats/${chatId}`, (data) => {
    if (data.gameMode == "match") {
      $("#chatName").text(data.postId.content);

      const countdownTimer = document.getElementById("timer");
      const minutes = parseInt(countdownTimer.getAttribute("data-minutes"));
      const seconds = parseInt(countdownTimer.getAttribute("data-seconds"));
      let timeRemaining = minutes * 60 + seconds;
      const countdown = setInterval(() => {
        const minsRemaining = Math.floor(timeRemaining / 60);
        const secsRemaining = timeRemaining % 60;
        countdownTimer.textContent = `${minsRemaining}:${secsRemaining
          .toString()
          .padStart(2, "0")}`;
        timeRemaining--;
